...
### 1. Spec-Driven TDD (REQUIRED)

**Pattern**: All development must follow the Spec-Driven TDD cycle.
**Process**:

1. **Spec**: Define objective and success criteria in `task.md`/`ImplementationPlan.md`.
2. **Red**: Create a failing test case (pytest or playwright).
3. **Green**: Implement minimum code to pass the test.
4. **Refactor**: Clean up and optimize while keeping tests green.
5. **Audit**: Run benchmarks for performance-critical features.

**Impact**: Ensures functional correctness and prevents regressions before they reach the codebase.
