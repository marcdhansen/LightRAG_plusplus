...
async def test_ace_reflector_repair_hallucination(ace_rag):
    ...
    await rag.chunk_entity_relation_graph.upsert_node("Mars", {"entity_type": "Location", "description": "Planet"})
    custom_rel = {
    ...
    await rag.chunk_entity_relation_graph.upsert_edge("Albert Einstein", "Mars", custom_rel)
    ...
    rel_exists = await rag.chunk_entity_relation_graph.has_edge("Albert Einstein", "Mars")
    ...
    print(f"✅ ACE Reflector identified and repaired the hallucination! Status: {repair_step['status']}")
