...
@pytest.mark.asyncio
async def test_unified_neighbor_search_call(
    mock_driver, mock_embedding_func, mock_data_init_lock
):
    ...
    # Initialize Storage
    with patch(
        "lightrag.kg.memgraph_impl.AsyncGraphDatabase.driver", return_value=driver
    ):
        kg = MemgraphStorage(
            namespace="test", global_config={}, embedding_func=mock_embedding_func
        )
        await kg.initialize()

        vdb = MemgraphVectorStorage(
            namespace="test",
            global_config={},
    ...
    with patch(
        "lightrag.operate._find_most_related_edges_from_entities",
...
@pytest.mark.asyncio
async def test_unified_edge_search_call(
    mock_driver, mock_embedding_func, mock_data_init_lock
):
    driver, session = mock_driver
    ...
    with patch(
        "lightrag.operate._find_most_related_entities_from_relationships",
        new_callable=AsyncMock,
    ) as mock_find_ents:
