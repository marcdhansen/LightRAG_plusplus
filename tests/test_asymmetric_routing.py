...
async def test_asymmetric_routing_logic():
    ...
    mock_hashing_kv.global_config = {"llm_model_name": "default-model"}
    ...
    # Case 1: No override
    with patch(
        "lightrag.llm.ollama._ollama_model_if_cache", new_callable=AsyncMock
    ) as mock_complete:
    ...
    with patch(
        ...
        await ollama_model_complete(
            prompt, hashing_kv=mock_hashing_kv, model="special-model"
        )
        mock_complete.assert_called_once()
...
async def test_lightrag_reflection_routing():
    ...
    gen_result = {
        ...
        "context_data": {"relationships": [], "chunks": []},
    }
