...
GOLD_CASES = [
    ...
    },
    {
        ...
        "expected_relations": [
            ...
            {"source": "Albert Einstein", "target": "Theory of Relativity", "keywords": ["developed", "developing"]},
        ]
    }
]
...
async def gold_rag():
    ...
    rag = LightRAG(
        ...
        addon_params={
            "entity_types": ["Person", "Location", "Organization", "Concept", "Theory", "Event"]
        },
...
async def test_gold_standard_extraction_cases(gold_rag):
    ...
    for case in GOLD_CASES:
        ...
        entity_map = {normalize_name(e["id"]): e["id"] for e in entities} # norm -> real_id
        ...
        for expected in case["expected_relations"]:
            ...
            src_real = next((real for norm, real in entity_map.items() if src_norm in norm), None)
            tgt_real = next((real for norm, real in entity_map.items() if tgt_norm in norm), None)

            if not src_real or not tgt_real:
                missing_relations.append(f"{expected['source']} -> {expected['target']} (Entities not found)")
                continue

            # Check Edge
            edge_exists = await rag.chunk_entity_relation_graph.has_edge(src_real, tgt_real)
            if not edge_exists:
                ...
                edge_exists = await rag.chunk_entity_relation_graph.has_edge(tgt_real, src_real)
        ...
        if case.get("xfail"):
             if missing_entities:
                 pytest.xfail(f"Expected failure for {case['id']}: Missing Entities {missing_entities}")
        
        assert not missing_entities, f"Case {case['id']} failed: Missing Entities {missing_entities}"
