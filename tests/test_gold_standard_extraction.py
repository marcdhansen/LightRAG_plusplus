...
async def test_gold_standard_extraction(gold_rag):
    ...
    assert any("EINSTEIN" in name.upper() for name in entity_names), "Missing Albert Einstein"
    # Check for Germany
    assert any("GERMANY" in name.upper() for name in entity_names), "Missing Germany"
    # Check for Relativity
    assert any("RELATIVITY" in name.upper() for name in entity_names), f"Missing Relativity in {entity_names}"
    ...
    relativity_name = next(name for name in entity_names if "RELATIVITY" in name.upper())

    # Verify Relationship: Einstein -> Germany
    exists_germany = await rag.chunk_entity_relation_graph.has_edge(ae_name, germany_name)
    assert exists_germany, f"Missing relationship between {ae_name} and {germany_name}"

    # Verify Relationship: Einstein -> Relativity
    exists_relativity = await rag.chunk_entity_relation_graph.has_edge(ae_name, relativity_name)
    assert exists_relativity, f"Missing relationship between {ae_name} and {relativity_name}"
