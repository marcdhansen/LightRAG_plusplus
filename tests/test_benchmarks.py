...
class TestRAGBenchmarks:
    ...
    async def test_hotpotqa_multihop(self, case):
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            payload = {"query": case["question"], "mode": "mix"}
            ...
            missing = [
                c
                for c in case["ground_truth_concepts"]
    ...
    async def test_chain_of_thought_reasoning(self, question):
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            ...
            if len(found_markers) < 1:
                ...
                print(
                    f"Warning: Response might lack explicit reasoning structure. Markers checked: {reasoning_markers}"
                )
            else:
