...
const fetchGraph = async (label: string, maxDepth: number, maxNodes: number) => {
  ...
  try {
    ...
    useGraphStore
      .getState()
      .addLog(
        `Fetched ${rawData?.nodes?.length || 0} nodes and ${rawData?.edges?.length || 0} edges.`
      )
  } catch (e) {
...
const useLightrangeGraph = () => {
  ...
  if (graphDataVersion !== prevGraphDataVersion.current) {
    ...
    console.log(
      `[useLightragGraph] Version changed to ${graphDataVersion}, resetting emptyDataHandledRef`
    )
  }
    ...
    const handleNodeExpand = async (nodeId: string | null) => {
      ...
      try {
        ...
        useGraphStore
          .getState()
          .addLog(
            `Fetched extension: ${extendedGraph.nodes.length} nodes, ${extendedGraph.edges.length} edges.`
          )
