...
class PerformanceMonitor:
    ...
    def record_metric(self, system: str, metric_type: str, value: float, metadata: Optional[Dict[str, Any]] = None) -> None:
    ...
    def get_average(self, system: str, metric_type: str, minutes: int = 5) -> Optional[float]:
        cutoff = datetime.now() - timedelta(minutes=minutes)
        recent_metrics = [
            m for m in self.metrics.get(system, [])
            if datetime.fromisoformat(m.get("timestamp", "")) > cutoff
            and m.get("type", "") == metric_type
        ]
        
        return sum(m.get("value", 0) for m in recent_metrics) / len(recent_metrics) if recent_metrics else None
...
enhanced_skills = [
    Skill(id="flight-director", name="FlightDirector", description="Flight planning and coordination", category="coordination", 
             examples=["Plan route", "Coordinate takeoff", "Monitor flight systems"], complexity=3, usage_count=15),
    Skill(id="librarian", name="Librarian", description="Knowledge management and retrieval", category="information",
             examples=["Search documents", "Organize knowledge", "Retrieve information"], complexity=2, usage_count=25),
    Skill(id="performance-optimizer", name="PerformanceOptimizer", description="System optimization and performance tuning", category="optimization",
             examples=["Analyze bottlenecks", "Optimize code", "Profile performance"], complexity=4, usage_count=20, parent_skill="flight-director"),
    Skill(id="security-expert", name="SecurityExpert", description="Security analysis and recommendations", category="security",
             examples=["Audit code", "Security review", "Penetration testing"], complexity=4, usage_count=8),
    Skill(id="data-scientist", name="DataScientist", description="Data analysis and machine learning", category="analytics",
             examples=["Statistical analysis", "Machine learning", "Data visualization"], complexity=5, usage_count=12),
    Skill(id="frontend-expert", name="FrontendExpert", description="Frontend development and optimization", category="engineering",
             examples=["React optimization", "UI/UX design", "Web performance"], complexity=4, usage_count=18, parent_skill="performance-optimizer"),
    Skill(id="backend-expert", name="BackendExpert", description="Backend development and scaling", category="engineering",
             examples=["API design", "Database optimization", "Microservices"], complexity=4, usage_count=15, parent_skill="performance-optimizer"),
    Skill(id="quality-analyst", name="QualityAnalyst", description="Standards for performance evaluation and testing", category="testing",
             examples=["Run benchmarks", "UI testing", "E2E verification"], complexity=3, usage_count=10),
    Skill(id="reflect", name="Reflect", description="Self-evolution and memory capture", category="learning",
             examples=["Session analysis", "Memory extraction", "Self-correction"], complexity=3, usage_count=30),
    Skill(id="coding-standards", name="CodingStandards", description="General coding best practices and patterns", category="engineering",
             examples=["Input validation", "Error handling", "Documentation standards"], complexity=2, usage_count=40)
]
...
async def create_embedding(request: EmbeddingRequest):
    ...
    performance_monitor.record_metric("openviking", "embedding_generation_time", processing_time)
...
async def search_skills(request: SkillSearchRequest):
    ...
    for skill in enhanced_skills:
        if (skill.category == category or category == "general") and \
           any(keyword in skill.name.lower() or keyword in skill.description.lower() 
               for keyword in query_lower.split()):
            # Add subskills if hierarchical search
            if hierarchical:
                related_skills = [s.id for s in enhanced_skills if s.parent_skill == skill.id]
                sub_skills = [s.name for s in enhanced_skills if s.parent_skill == skill.id and s.subcategory]
            else:
                related_skills = []
                sub_skills = []
    ...
    performance_monitor.record_metric("openviking", "skill_search_time", processing_time)
...
async def handle_conversation(request: ConversationRequest):
    ...
    context_messages = conversation_memory.get_context(request.session_id, request.context_turns)
...
async def flush_sessions(session_id: Optional[str] = None):
...
async def add_resource(request: ResourceRequest):
    ...
    processing_time = (time.time() - start_time) * 1000
    performance_monitor.record_metric("openviking", "resource_storage_time", processing_time)
...
async def get_performance_metrics(system: Optional[str] = None, minutes: int = 5):
    """Real-time performance monitoring dashboard"""
    if system and system in performance_monitor.metrics:
        cutoff = datetime.now() - timedelta(minutes=minutes)
        recent_metrics = [
            m for m in performance_monitor.metrics[system]
    ...
    return {
        ...
        "system": system,
        "averages": performance_monitor.get_average(system, "embedding_generation_time", 5) if system else None,
...
async def performance_dashboard():
    ...
    for system, metrics in performance_monitor.metrics.items():
        system_averages = {}
        for metric_type in ["embedding_generation_time", "skill_search_time", "resource_storage_time"]:
    ...
    return {
        ...
        "systems": all_metrics,
        "summary": {
            "total_requests": sum(len(metrics) for metrics in performance_monitor.metrics.values()),
            "cache_hit_rate": performance_monitor.get_average("openviking", "embedding_cache_hit", 5) or 0.0
        }
    }
